...
from stochastic_primal_dual_hybrid_gradient import (spdhg, pa_spdhg,
                                                    spdhg_pesquet)
import misc

# create folder structure and set parameters
folder_out = '.'  # to be changed
filename = 'example_rof'
nepoch = 300
niter_target = 2000
...
if not os.path.exists(file_target):
    ...
    misc.save_image(suby_opt[0], 'suby_saddle[0]', folder_main, 4)
...
bregman_g = g.bregman(x_opt, subx_opt)
...
for alg in ['pdhg', 'pesquet_uni2', 'pa_pdhg', 'spdhg_uni2', 'pa_spdhg_uni2']:
    ...
    n = nsub[alg]
    (sub2ind, ind2sub) = misc.divide_1Darray_equally(range(2), n)
