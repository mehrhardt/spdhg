...
g = misc.TotalVariationNonNegative(X, alpha=2e-1)
...
if not os.path.exists(file_target):
    ...
    misc.save_image(subx_opt, 'q_saddle', folder_main, 3)
    misc.save_signal(suby_opt[0], 'r_saddle[0]', folder_main, 4)
...
bregman_g = g.bregman(x_opt, subx_opt)  # primal Bregman distance

# dual Bregman distance
bregman_f = odl.solvers.SeparableSum(
    *[fi.convex_conj.bregman(yi, ri)
...
label = ['PDHG', 'SPDHG (10 subsets)', 'SPDHG (50)',
         'Pesquet\&Repetti (10)', 'Pesquet\&Repetti (50)']
fig = []
...
for k, a in enumerate(algs[:3]):
    ...
    plt.loglog(x[i], y[i], color=colors[k], linestyle=lstyle,
               linewidth=lwidth,
